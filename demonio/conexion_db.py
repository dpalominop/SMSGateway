...
" % (nombre, codigo, fecha_init, fecha_fin, hora_init, hora_fin)
            ...
            try:
                ...
                for mensaje in sms_revisados:
                    ...
                    sql = """
                            update mensajes set gateway_id = %d,
                            estado = %d,
                            error = '%s',
                            intentos_fallidos = %d,
                            activado = 0,
                            ...
                            hora_de_encola = '%s'
                            where id = %d
                    """ % (Null_if_neg(mensaje.message_id),
                            Null_if_neg(mensaje.estado),
                            mensaje.error,
                            mensaje.failures,
                            mensaje.hora_de_envio,
                            mensaje.hora_de_encola,
                            mensaje.indice)
            ...
            if str(fecha_fin) <= time.strftime("%Y-%m-%d") or (len(mensajes) == len(sms_revisados)):
                ...
                sql = """
                            update campanias set activado = 0 where id = %d
                    """ % (codigo)
